...
uint32_t super_new_block() {
	...
	if (block_taken_data.free_needToWrite) {
		super_save_free(super.firstFreeBlock, block_taken_data.free_data);
	}
...
void super_free_block(uint32_t block) {
	...
	if (free_block_put_ret.free_writeAt0 != 0) {
		super_save_free(free_block_put_ret.free_writeAt0, free_block_put_ret.free_data0);
	}
	
	if (free_block_put_ret.free_writeAt1 != 0) {
		super_save_free(free_block_put_ret.free_writeAt1, free_block_put_ret.free_data1);
	}
